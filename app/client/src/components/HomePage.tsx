    ...
    <div className="flex h-screen w-screen flex-col overflow-hidden max-w-screen-xl mx-auto">
          ...
          <div className="w-full max-w-5xl flex flex-wrap justify-center gap-8">
            {budgetSummaries.map((b) => (
              <div key={b.id} className="bg-blue-600 text-white rounded-2xl shadow-lg p-6 w-80 flex flex-col justify-between">
                ...
                <Link to={`/b/${b.shortCode}`} className="mt-auto px-4 py-2text-blue-600 text-center font-medium rounded-xl hover:bg-blue-100 transition">
                  Open
                </Link>
              </div>
            ))}
      ...
      {!showWizard ? (
        <div className="text-center my-8">
          <button className="px-6 py-3 bg-blue-600 rounded-xl shadow hover:bg-blue-700 transition" onClick={() => setShowWizard(true)}>
            Create budget...
          </button>
        </div>
      ) : (
